...
public abstract class RobotRun extends Thread {
	...
	public void run() {
		...
		RobotStructure.getInstance().leftWheel
				.synchronizeWith(new RegulatedMotor[] { RobotStructure.getInstance().rightWheel });
	...
	/**
	 * The contents of the run which should be implemented on a per run basis. Note
	 * that the instruction should be able to handle interrupts and stop the run
	 * accordingly.
	 */
